...
export function App() {
  ...
  const [airdropType, setAirdropType] = useState<AirdropTypeEnum>(AirdropType.ETH);
    ...
    <div className={tw.h_screen.bg_transparent}>
      ...
      {isConnected && (
        ...
        <div className={tw.m_10}>
          <center>{airdropType === AirdropType.ERC20 ? <ERC20Provider /> : <ETHProvider selected={airdropType} setSelected={setAirdropType} />}</center>
        </div>
      )}
    </div>
  );
